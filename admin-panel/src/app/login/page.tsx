...
export default function LoginPage() {
  ...
  useEffect(() => { const token = getToken(); if (token) verify(token).then(() => router.replace('/')).catch(() => undefined); }, [router]);
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setError(''); setLoading(true);
    try { await login(mobile, password); router.replace('/'); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Unable to sign in.'); } finally { setLoading(false); }
  }
