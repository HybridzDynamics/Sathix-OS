...
const primaryMetrics = [['totalSchemes', 'Total Schemes', FileText], ['totalUsers', 'Total Users', Users], ['aiQueries', 'AI Queries', Bot], ['scraperRuns', 'Scraper Runs', Activity]] as const;

export default function DashboardPage() {
  const [overview, setOverview] = useState<AdminOverview | null>(null); const [error, setError] = useState(''); const [loading, setLoading] = useState(true); const [attempt, setAttempt] = useState(0);
  useEffect(() => { let active = true; getOverview().then((result) => { if (active) setOverview(result); }).catch((cause) => { if (active) setError(cause instanceof ApiError ? cause.message : 'Unable to load the operational overview.'); }).finally(() => { if (active) setLoading(false); }); return () => { active = false; }; }, [attempt]);
